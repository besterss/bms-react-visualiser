  ...
  createStairs(
    ...
    if (!isLastPart) {
      ...
      if (railing === "yes") {
        ...
        railingMaterial.diffuseColor = new BABYLON.Color3(1, 0.75, 0.8); // Light pink for transparency effect
        railingMaterial.alpha = 0.5; // For a glassy effect
        const railingHeight = 1.0;
        const railingThickness = 0.05;
        for (let i = 0; i < numSteps; i++) {
          const yPosition = startingYLevel + i * stepHeight + railingHeight / 2;
          // Railing on the left side of steps
          const railingLeft = BABYLON.MeshBuilder.CreateBox(
            `stair_railing_left_${i}`,
            {
          ...
          railingLeft.position.y = yPosition;
          ...
          // Railing on the right side of steps
          const railingRight = BABYLON.MeshBuilder.CreateBox(
            `stair_railing_right_${i}`,
            {
              width: railingThickness,
              height: railingHeight,
              depth: stepDepth,
          ...
          railingRight.position.x =
            position.x + stepWidth / 2 + railingThickness / 2;
          railingRight.position.y = yPosition;
        ...
        // Optionally add railing at the floor ends if required
        if (!isLastPart) {
          const floorEndYPosition =
            startingYLevel + numSteps * stepHeight + railingHeight / 2;
          // Railing on the left side of the floor end
          const floorRailingLeft = BABYLON.MeshBuilder.CreateBox(
            `floor_railing_left`,
            {
              width: railingThickness,
              height: railingHeight,
          ...
          floorRailingLeft.position.x =
            floorMesh.position.x - floorWidth / 2 - railingThickness / 2;
          floorRailingLeft.position.y = floorEndYPosition;
          floorRailingLeft.position.z = floorMesh.position.z;
          floorRailingLeft.material = railingMaterial;
          stairMeshes.push(floorRailingLeft);
          // Railing on the right side of the floor end
          const floorRailingRight = BABYLON.MeshBuilder.CreateBox(
            `floor_railing_right`,
            {
              width: railingThickness,
              height: railingHeight,
          ...
          floorRailingRight.position.x =
            floorMesh.position.x + floorWidth / 2 + railingThickness / 2;
          floorRailingRight.position.y = floorEndYPosition;
          floorRailingRight.position.z = floorMesh.position.z;
          floorRailingRight.material = railingMaterial;
          stairMeshes.push(floorRailingRight);
        }
      }
    }
